...
# Settings
flags = tf.app.flags
FLAGS = flags.FLAGS  # 用于接收从终端传入的命令行参数
...
def gae_for_na(name):
    ...
    # dia_matrix((data, offsets), shape=(m, n))，生成大小为mxn的稀疏矩阵
    # np.newaxis的作用是增加一个维度
    # diagonal()是获得矩阵对角线
    # adj_orig.diagonal()[np.newaxis, :], [0]是先将对角线提取出来然后增加一维变为矩阵
...
def main():
    names = load_test_names()  # 加载测试作者名列表
